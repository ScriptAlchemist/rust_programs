...
pub fn convert_v5(s: String, num_rows: i32) -> String {
   String::from_utf8((0..num_rows as usize).chain((1..(num_rows-1) as usize).rev()).cycle().zip(s.bytes()) //prep the index, value pair, lazy
        .fold(vec![vec![]; num_rows as usize], |mut acc, (idx, byte)| { acc[idx].push(byte); acc}) //accumulate the bytes properly
        .into_iter().flatten().collect::<Vec<u8>>()).unwrap() //flatten and convert to a string 
}
